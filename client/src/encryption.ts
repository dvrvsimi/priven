...
export async function encryptPredicate(
  ...
  if (ciphertexts[0].length !== 32) {
    throw new Error(
      `Invalid ciphertext length: expected 32 bytes, got ${ciphertexts[0].length}`
    );
  }
 ...
 */
function serializePredicate(predicate: Predicate): bigint[] {
 ...
 */
function parseQueryResult(plaintext: bigint[]): QueryResult {
