...
export class PrivenClient {
  ...
  async query(
    ...
    const computationOffset = await this.submitQuery(
      encrypted,
      selectedPools.map((p) => p.address)
    );
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("MPC computation timeout")),
            timeout
          )
        ),
      ]);
  ...
  private async submitQuery(
    encrypted: { ciphertext: Uint8Array; publicKey: Uint8Array; nonce: bigint },
    ...
    try {
      ...
      const signature = await this.program.methods
        .submitQuery(
          computationOffset,
          Array.from(encrypted.ciphertext), // [u8; 32]
  ...
  private async fetchAndDecryptResults(
    ...
    const queryResult: any = await (this.program.account as any).queryResultAccount.fetch(
      queryResultPda
    );
    ...
    const encryptedResult = new Uint8Array(
      queryResult.encryptedResult.slice(0, encryptedResultLen)
    );
