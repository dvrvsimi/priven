...
exports.buildTokenChangeFilter = buildTokenChangeFilter;
exports.createStreamsClient = createStreamsClient;
exports.buildPrivenFilter = buildPrivenFilter;
...
class StreamsClient {
     ...
     */
    async createTokenMonitorStream(config) {
        // Build filter function (runs on QuickNode's infrastructure)
        const filterFunction = buildTokenChangeFilter(config.tokenMint?.toBase58(), config.minBalanceChange);
        ...
        const response = await fetch(`${QUICKNODE_API_BASE}/streams/rest/v1/streams`, {
            method: "POST",
            headers: {
     ...
     */
    async listStreams() {
        const response = await fetch(`${QUICKNODE_API_BASE}/streams/rest/v1/streams?limit=100`, {
    ...
    async getStream(streamId) {
        const response = await fetch(`${QUICKNODE_API_BASE}/streams/rest/v1/streams/${streamId}`, {
            headers: { "x-api-key": this.apiKey },
        });
    ...
    async pauseStream(streamId) {
        const response = await fetch(`${QUICKNODE_API_BASE}/streams/rest/v1/streams/${streamId}/pause`, {
            method: "POST",
            headers: { "x-api-key": this.apiKey },
        });
        if (!response.ok) {
     ...
     */
    async activateStream(streamId) {
        const response = await fetch(`${QUICKNODE_API_BASE}/streams/rest/v1/streams/${streamId}/activate`, {
            method: "POST",
            headers: { "x-api-key": this.apiKey },
        });
        if (!response.ok) {
     ...
     */
    async testFilter(network, blockNumber, filterFunction) {
        const filterBase64 = Buffer.from(filterFunction).toString("base64");
        const response = await fetch(`${QUICKNODE_API_BASE}/streams/rest/v1/streams/test_filter`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "x-api-key": this.apiKey,
 ...
 */
function buildTokenChangeFilter(tokenMint, minBalanceChange) {
 ...
 */
function createStreamsClient(apiKey) {
    const key = apiKey || process.env.QUICKNODE_API_KEY;
    if (!key) {
        throw new Error("QUICKNODE_API_KEY environment variable or apiKey parameter required");
    }
 ...
 */
function buildPrivenFilter(config = {}) {
